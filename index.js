...
RULES:
1. NEVER greet. No Hello, Hi, Namaste, Good morning. Answer directly.

2. LANGUAGE RULE — THIS IS MOST IMPORTANT:
   - Detect the EXACT language the guest used
   - If guest wrote in ENGLISH → reply ONLY in English
   - If guest wrote in HINDI (Devanagari script) → reply ONLY in Hindi
   - If guest wrote in Hinglish (Hindi words in English letters) → reply in Hinglish
   - NEVER switch languages. Match exactly what language guest used.

3. HOTEL questions: answer from hotel info above. If missing say call reception.

4. CITY/TRAVEL questions: use your own knowledge, give real answers:
   - Places in Udaipur: City Palace, Lake Pichola, Fateh Sagar, Sajjangarh, Jagdish Temple
   - Local food: Dal Baati Churma, Laal Maas, Ghewar, Kachori
   - Shopping: Hathi Pol Bazaar, Clock Tower Market

5. SERVICE REQUESTS needs_staff true:
   - Wake up call → Front Desk, task: Wake up call Room ${room_number} at [time]
   - Towels/pillow → Housekeeping
   - Food order → Room Service
   - AC/TV broken → Maintenance

6. Emergency → needs_staff true, priority urgent.

JSON only, no markdown:
{"reply":"answer in SAME language as guest","needs_staff":false,"department":null,"task":null,"priority":"normal","english_message":"english translation"}`;
...
app.post('/upload-logo', async (req,res) => {
  try {
    const { image_base64, hotel_id, file_type } = req.body;
    if (!image_base64 || !hotel_id) return res.status(400).json({ error: 'Missing data' });
    
    // Convert base64 to buffer
    const base64Data = image_base64.replace(/^data:image\/\w+;base64,/, '');
    const buffer = Buffer.from(base64Data, 'base64');
    const ext = file_type === 'image/png' ? 'png' : file_type === 'image/webp' ? 'webp' : 'jpg';
    const fileName = hotel_id + '_logo_' + Date.now() + '.' + ext;
    ...
    // Update hotel logo_url with the public URL
    await supabase.from('hotels').update({ logo_url: publicUrl }).eq('hotel_id', hotel_id);
...
// FAST SINGLE HOTEL ENDPOINT - excludes heavy base64 data
app.get('/hotel', async (req,res) => {
  const hotel_id = req.query.hotel_id;
  if (!hotel_id) return res.status(400).json({ error: 'hotel_id required' });
  const { data, error } = await supabase.from('hotels')
    .select('hotel_id,name,city,colour,emoji,hotel_photos,dept_guest_options,guest_options,ai_kb,menu,room_guide,hotel_code')
    .eq('hotel_id', hotel_id).single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
